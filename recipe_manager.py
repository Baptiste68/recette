...
class RecipeManager:
    ...
    def formater_recettes(self, recettes_brutes: List[Dict], inventaire: Dict) -> str:
        ...
        if self.regimes_actifs:
            regimes_str = ", ".join([r.value[0].replace('-', ' ').title() for r in self.regimes_actifs])
    ...
    def obtenir_details_recette(self, recipe_id: int) -> Tuple[bool, str, Dict]:
        """
        Récupère les détails complets d'une recette
        
        Returns:
            Tuple[bool, str, Dict]: (succès, message, détails_recette)
        """
        try:
            url = f"{self.base_url}/recipes/{recipe_id}/information"
            ...
            if response.status_code == 200:
                ...
                return True, "Détails récupérés avec succès", details
            
            elif response.status_code == 402:
                return False, "Limite d'API atteinte", {}
            else:
                return False, f"Erreur API: {response.status_code}", {}
